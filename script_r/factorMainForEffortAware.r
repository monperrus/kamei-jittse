...
coreExperiment <- function(fit.name, est.name, fit, est, formula, cutoffs, model, sampling=F, filter=T){
  ...
  # Output Coefficient
  out.fname <- paste(fit.name,"_factor_effort.csv",sep="")
  write.csv(t(lr$coe), file=file.path("..", "output", out.fname, fsep = .Platform$file.sep))
...
#out.fname <- paste("result_", target, "_effortaware.csv",sep="")
